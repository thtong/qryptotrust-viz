 ...
 * secret keys never leave localStorage. Server endpoints (under /api/mw/*)
...
(function () {
    ...
    async function ensurePrekeyHealth(deviceId) {
        try {
            const r = await fetch('/api/mw/prekeys/' + encodeURIComponent(deviceId) + '/count');
            if (!r.ok) return;
    ...
    async function generateAndRegisterDevice(email, deviceName) {
        ...
        if (existing && existing.deviceId) {
            ...
            if (dev) {
                ...
                // been wiped by DELETE /api/mw/accounts/<email>). If gone,
                // purge the stale local state and fall through to a fresh
                // registration below.
                let stillOnServer = null;
                try {
                    const r = await fetch('/api/mw/devices/' + encodeURIComponent(email));
                    if (r.ok) {
                ...
                if (stillOnServer === true) {
                    ...
                    try {
                        await fetch('/api/mw/groups/accept-all/' + encodeURIComponent(email), { method: 'POST' });
                    } catch (_) { /* non-fatal */ }
        ...
        // Register on server
        const regResp = await fetch('/api/mw/devices', {
            method: 'POST',
        ...
        try {
            await fetch('/api/mw/groups/accept-all/' + encodeURIComponent(email), {
                method: 'POST',
            });
        } catch (_) { /* non-fatal */ }
    ...
    async function rotateSignedPrekey(deviceId) {
        ...
        const resp = await fetch('/api/mw/prekeys/upload', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                device_id: deviceId,
    ...
    async function establishOutboundSession(myDeviceId, recipientDeviceId) {
        ...
        const bundleResp = await fetch(
            '/api/mw/prekeys/' + encodeURIComponent(recipientDeviceId) + '/take'
        );
        if (!bundleResp.ok) {
    ...
    async function pcsStep(myDeviceId, peerDeviceId) {
        ...
        const bundleResp = await fetch(
            '/api/mw/prekeys/' + encodeURIComponent(peerDeviceId) + '/take'
        );
    ...
    async function encryptForRecipientEmail(myDeviceId, recipientEmail, plaintext) {
        ...
        const devsResp = await fetch(
            '/api/mw/devices/' + encodeURIComponent(recipientEmail)
        );
    ...
    async function authorizedDeviceAcceptLinkingCode(myDeviceId, code) {
        ...
        const authResp = await fetch('/api/mw/devices/authorize', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                device_id: code.deviceId,
    ...
    async function newDeviceCompleteLinking(deviceId) {
        ...
        const devsResp = await fetch('/api/mw/devices/' + encodeURIComponent(pending.email));
